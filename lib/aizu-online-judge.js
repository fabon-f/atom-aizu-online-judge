...
body {
  ...
  background-color: ${getComputedStyle(problemViewerElement).backgroundColor};
}
</style>
</head>
<body>
${description}
        ...
        descriptionViewer.classList.add('description', 'hidden-webview');
        ...
        descriptionViewer.addEventListener('ipc-message', e => {
          ...
          descriptionViewer.classList.remove('hidden-webview');
        });
        ...
        descriptionViewer.addEventListener('dom-ready', () => {
          const openLinkInBrowser = ({ url }) => {
            const protocol = parseURL(url).protocol;
            if (protocol !== 'http:' && protocol !== 'https:') { return; }
            shell.openExternal(url);
          };
          descriptionViewer.addEventListener('did-start-loading', () => {
            descriptionViewer.stop();
          });
          descriptionViewer.addEventListener('new-window', openLinkInBrowser);
          descriptionViewer.addEventListener('will-navigate', openLinkInBrowser);
        });
